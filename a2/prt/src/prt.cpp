...
NORI_NAMESPACE_BEGIN
    ...
    public:
                        ...
                        if (intersects) {
                            ...
                            Normal3f interpNormal =
                                    Normal3f(normals.col(triangleIndexs.x()).normalized() * bary.x() +
                                            ...
                                            .normalized();
                            auto nextBounce = SumIndirectCoeffs(directCoeffs, scene, intersectPoint, interpNormal,
                                                                depth + 1);
        ...
        Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
            ...
            const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> rL = m_LightCoeffs.row(0), gL = m_LightCoeffs.row(
                    1), bL = m_LightCoeffs.row(2);
